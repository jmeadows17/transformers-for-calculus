    ...
    for i in D:
        for j in D:
            if str(i) != str(j):
                ...
                if (type(LHS) is not sp.core.numbers.One) and (type(LHS) is not sp.core.numbers.Zero) and (type(RHS) is not sp.core.numbers.Zero) and (type(RHS) is not sp.core.numbers.One):
...
def step(D, p_history=10, p_arity_0=5, p_renaming=100, p_arity_1=50, p_evaluate=50, p_arity_2=100, p_int_or_diff=2, p_subs=100):
...
def length():
    while True:
        ...
        length = int(np.round(np.random.normal(6.5, 3, 1)))
...
def derivation():
    ...
    while True:
        ...
        if (next_step != False) and (next_step not in D) and (next_step[1] not in eval_ints):
            D.append(next_step) 
...
def contains_bad_func_arguments(eq):
    functions = [i for i in get_equation_elements(eq) if type(type(i)) is sp.core.function.UndefinedFunction]
    for f in functions:
        func_args = get_equation_elements(f)
        bad_list = [i for i in func_args if type(type(i)) is sp.core.function.UndefinedFunction]
        if bad_list:
            return True
    return False
